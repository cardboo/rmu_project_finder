...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("SELECT password FROM admin_logs WHERE username = ?");
    ...
    $update = $conn->prepare("UPDATE admin_logs SET password = ? WHERE username = ?");
    $update->bind_param("ss", $hashed, $username);

    if ($update->execute()) {
        audit_log($conn, 'password_changed', "Admin changed their password");
