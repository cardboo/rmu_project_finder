...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email)) {
        ...
        // Look up department by email
        $stmt = $conn->prepare("SELECT id, dep_name, username FROM departments WHERE email = ? AND is_archived = 0");
        ...
        if ($result->num_rows === 1) {
            ...
            $del = $conn->prepare("DELETE FROM password_resets WHERE user_type = 'department' AND user_id = ?");
            $del->bind_param("i", $dept['id']);
            $del->execute();
            $del->close();

            // Insert new token
            $ins = $conn->prepare("INSERT INTO password_resets (user_type, user_id, token, expires_at) VALUES ('department', ?, ?, ?)");
            $ins->bind_param("iss", $dept['id'], $token, $expires);
            $ins->execute();
            $ins->close();
            ...
            try {
                ...
                $mail->addAddress($email, $dept['dep_name']);

                $mail->isHTML(true);
                $mail->Subject = 'Password Reset - RMU Project Finder';
                $mail->Body = "
                    <h3>Password Reset Request</h3>
                    <p>Hello <b>{$dept['dep_name']}</b>,</p>
                    <p>We received a request to reset your department account password.</p>
                ...
                error_log("PHPMailer error (dep_admin forgot_password): " . $e->getMessage());
            }
        }
...
<head>
  ...
  <title>Forgot Password - Department</title>
          ...
          <div class="col-md-8 col-lg-6 col-xxl-3">
            <div class="card mb-0">
              <div class="card-body">
                ...
                <p class="text-center">Department Password Reset</p>
