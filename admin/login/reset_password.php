...
if (empty($token)) {
    ...
    // Verify token
    $stmt = $conn->prepare("SELECT id, user_id FROM password_resets WHERE token = ? AND user_type = 'admin' AND used = 0 AND expires_at > NOW()");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $validToken) {
    ...
    if (strlen($password) < 8) {
        ...
        $upd = $conn->prepare("UPDATE admin_logs SET password = ? WHERE t_id = ?");
